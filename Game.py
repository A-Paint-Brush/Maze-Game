    ...
    def game(self):
        while self.gameRun:
            ...
            if self.wall:
                ...
                self.bubble = True
                self.create_text_bubble(["You did it!", "You Win!"], (self.cat.pos_x - 55, self.cat.pos_y + 50), 3, "right")
            elif self.Exit:
                self.bubble = True
                self.create_text_bubble(["Scratch Cat is so badly injured he can't walk anymore.", "You Lose!"],
                                        (20, 60), 3, "left")
            if self.Exit:
                if not self.bubble:
                    self.gameRun = False
